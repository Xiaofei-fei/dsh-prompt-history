/**
 * InputHistory: bash-like prompt history for the composer (dsh-prompt-history).
 * Renders nothing — it mounts one capture-phase keydown listener on the
 * document while the session's composer card is live. Up recalls previously
 * submitted prompts (newest first), Down walks forward and restores the line
 * that was being typed before browsing began; editing the draft while browsing
 * drops back to the live line. The '/' and '@' suggestion menus keep their own
 * arrow-key navigation: while the menu (role=listbox inside the composer card)
 * is open, this listener declines and the input trigger pipeline owns the keys.
 *
 ...
 * adjudicating/submitting, session not removed, and a non-empty history.
 */
...
/**
 * The composer history entry: no visible UI, one keydown listener.
...
export function InputHistory({ useInput, useSession, inputActions, sessionId }: InputHistoryProps) {
  // Latest machine/session facts at event time (the listener mounts once).
