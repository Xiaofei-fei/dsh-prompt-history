...
- **选中即复制（全页面）**：页面里任何非空选区——输入框（拖选、双击选词、键盘 `Shift+方向键`、`Ctrl+A`）、聊天消息、代码块等——选区稳定后自动复制到剪贴板，并在选区上方闪现「已复制」提示；输入框走自身复制处理（chips 投影一致），execCommand 被限制时回退 Clipboard API
...
打开 **设置 → 终端式输入**，可独立开关两个行为（默认全部开启，存于浏览器本地，关闭立即生效）：

- **选中即复制**：关闭后任何选中都不再自动复制
- **右键直接粘贴**：关闭后右键恢复浏览器原生菜单
