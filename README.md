...
- **复制 + 引用（三档模式，设置里切换）**：页面里任何非空选区——输入框、聊天消息、代码块等——按所选模式处理：**工具栏**（默认，选区上方出现「复制」和「引用」按钮——复制点击才写剪贴板；引用把选中文本作为**引用 chip**（「引用：…」标签）插入输入框，视觉明显区分，发送时自动展开为 `>` 引用块，chip 后自动换行）/ **选中即自动复制**（终端风格，直接写系统剪贴板，会刷屏 Win+V 历史）/ **关闭**
