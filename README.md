...
- **复制 + 引用（三档模式，设置里切换）**：页面里任何非空选区——输入框、聊天消息、代码块等——按所选模式处理：**工具栏**（默认，选区上方出现「复制」和「引用」按钮——复制点击才写剪贴板；引用把选中文本的**完整内容**以 `>` 引用块直接插入输入框（前后空行分隔、完全可见可编辑，Codex 风格））/ **选中即自动复制**（终端风格，直接写系统剪贴板，会刷屏 Win+V 历史）/ **关闭**
