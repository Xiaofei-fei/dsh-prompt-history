...
- **Copy + quote (three modes, in Settings)**: any non-empty selection in the page — the composer textarea, chat messages, code blocks — is handled per the chosen mode: **toolbar** (default: 复制 / 引用 / 代码 buttons appear above the selection — copy writes the clipboard only on click; quote inserts the FULL selected text as a clean `>`-prefixed markdown blockquote; 代码 copies the selection wrapped in a ``` fenced code block) / **auto** (terminal-style: copies the selection straight to the system clipboard; floods Win+V history) / **off**.
- **Cross-session history** (Settings toggle, default off): keeps ↑/↓ history across sessions, stored in browser localStorage (cap 200), survives reloads and session switches.
