...
- **Copy + quote (three modes, in Settings)**: any non-empty selection in the page — the composer textarea, chat messages, code blocks — is handled per the chosen mode: **toolbar** (default: 复制 and 引用 buttons appear above the selection — copy writes the clipboard only on click; quote inserts the FULL selected text into the composer wrapped in a card-style box (`┌─ 引用 ─┐ / │ content │ / └──┘` borders, `>`-prefixed lines) — fully visible and editable, clearly distinct from normal input, rendered as a blockquote on send; the next input starts on the line directly below the card) / **auto** (terminal-style: copies the selection straight to the system clipboard; floods Win+V history) / **off**.
