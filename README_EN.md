...
- **Copy + quote (three modes, in Settings)**: any non-empty selection in the page — the composer textarea, chat messages, code blocks — is handled per the chosen mode: **toolbar** (default: 复制 and 引用 buttons appear above the selection — copy writes the clipboard only on click; quote inserts the selection into the composer as a visually distinct reference chip with a full-content preview strip above the composer; it expands to a `>` blockquote on send, with an automatic line break after the chip) / **auto** (terminal-style: copies the selection straight to the system clipboard; floods Win+V history) / **off**.
